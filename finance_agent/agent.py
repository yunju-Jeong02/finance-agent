...
from finance_agent.nodes.report_builder_node import ReportBuilderNode
...
class FinanceAgent:
    ...
    def __init__(self):
        ...
        self.report_builder_node = ReportBuilderNode()
    ...
    def _build_graph(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            ...
            {
                "end": END,
                "report_builder": "report_builder",   # [ADD]
    ...
    def report_builder(self, state: GraphState) -> GraphState:
        return self.report_builder_node.process(state)
    ...
    def route_after_query_parser(self, state: GraphState) -> str:
        if state.get("is_complete", False):
            return "end"
        intent = (state.get("parsed_query") or {}).get("intent", "")
        # [ADD] 보고서 의도면 보고서 노드로
        if intent in ("daily_report_request", "weekly_report_request"):
