...
'''
import re, json, pymysql, requests, uuid
import pandas as pd
from datetime import datetime
from typing import Dict
from config.config import Config
...
class QueryParserNode:
    ...
    def get_day_label(self, date: datetime) -> str:
    ...
    def process(self, state: Dict) -> Dict:
        ...
        if intent != "not_summary":
            date = extract_date(query) if intent != "url_summary_request" else ""
            # 키워드에서 날짜 문자열 제거
            raw_keywords = extract_keywords(query)
            keywords = [kw for kw in raw_keywords if not re.search(r'\d{4}[-./]?\d{1,2}[-./]?\d{0,2}', kw)]
            state["parsed_query"] = {
                "intent": intent,
                "date": date,
                "keywords": keywords,
