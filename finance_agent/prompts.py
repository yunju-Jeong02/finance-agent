...
1. 주어진 [대화 기록]과 [사용자 질문]의 전체 맥락을 바탕으로 질문이 모호한지 판단합니다.
...
[규칙]
- 날짜 혹은 기간이 명시되어야 하는 경우, "최근" 등의 표현은 모호하므로 명확한 날짜나 기간을 요청해야 합니다. "어제", "한달" 등의 표현은 시작날짜를 오늘로 간주하여 구체적인 날짜로 변환할 수 있어 추가 정보가 필요없습니다.
...
[규칙]
- ✨ 대화 기록에 회사명이 이미 언급되었다면, 현재 질문에 회사명이 없더라도 추출해야 합니다. ✨
...
<SQL 쿼리 작성 규칙>
1. 날짜는 항상 YYYY-MM-DD 포맷. 날짜가 없으면 최신 날짜 {latest_date} 사용. "한달", "일주일"과 같은 표현은 범위로 간주하여 계산.
...
news_parser_prompt="""
뉴스 요약 관련 질문일 경우, 아래 정보를 추출하여 JSON으로 추출하세요.
1. 날짜: 특정 날짜가 아닌 범위가 주어진 경우, "" 반환
2. 회사명: 해당 회사를 잘 몰라도 질문의 맥락 상 회사명이라고 판단되는 경우 추출 
3. 키워드 : 날짜, 회사명 제외 질문에서 핵심 키워드 여러 개 추출, 없으면 "" 반환
...
news_summary_prompt = """
다음 뉴스 기사(또는 뉴스 본문)를 간결하게 요약해 주세요.
...
당신은 금융 데이터 분석가입니다. 아래 제공된 특정 회사의 지난 일주일간 뉴스 기사들을 바탕으로, 회사의 주요 동향, 긍정적/부정적 이슈, 그리고 향후 전망을 간결하게 요약하는 주간 보고서를 작성해주세요.

보고서는 다음 형식에 맞춰 개조식으로 작성해주세요:
- **주요 동향:**
- **핵심 이슈:**
- **종합 의견:**
